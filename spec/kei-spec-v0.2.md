...
### 2.4 パッケージ束縛(M35 / v0.6)

`extern` 署名の名前空間は §2.1 では `import` した Kei モジュールのエイリアスに限られていた。
npm パッケージ(Cloudflare Workers / Node 上のライブラリ)を同じ枠組みで扱えるよう、
`extern package` 宣言で npm bare specifier を名前空間に束縛できる。
...
- `extern package "<specifier>" as <束縛名>` を import 群と同じモジュール先頭領域に置く。
- `<束縛名>` は以降のモジュール内で **`extern <束縛名>.<関数>(...)` 署名の名前空間としてのみ**
  使用できる。§2.1 の `extern` 署名と組み合わせて初めて意味を持つ。
...
`<束縛名>` は extern 署名の名前空間としてのみ使用可能で、以下はすべて診断で拒否する
(`KEI-E3007`):

- 値としての単体参照(`let x = hono`)
- 呼び出しを伴わないフィールド参照(`hono.foo`)
- 型注釈位置での使用(`let x: hono.Foo`)
- record リテラル(`hono { ... }` / `hono.Foo { ... }`)
- 裸呼び出し(`hono(...)`)

同一束縛名の重複宣言は既存の import 名前衝突と同じ診断に乗る。同一 specifier を
別の束縛名で複数回宣言することは許可される(診断なし)。

#### emit

`extern package "hono" as hono` は宣言ごとに TS の namespace import 1 行を出力する:

```ts
import * as hono from "hono";
```

出力位置は既存の `import` 群の直後(段階1: 常時出力、未使用検出はしない。既存の
`import` の emit と同じ方針)。

#### strict-extern との関係

束縛名配下の `extern` 署名(`extern hono.text(...)`)は、§2.3 の strict-extern 機構が
そのままそのメンバー呼び出しに適用される。未宣言のメンバー呼び出しは通常の `import`
名前空間と同様に `KEI-E3004`(warning)の対象になる。

#### opaque_imports との関係(MCP `kei_check`)

MCP `kei_check` ツールの `opaque_imports` はソーステキストのみから見える `import`
宣言の遷移先モジュールパスを列挙するものであり、`extern package` 束縛はここに含まれない。
`extern package` は `extern <束縛名>.<関数>(...)` 署名を伴って初めて意味を持ち、その署名が
型とエフェクトを既に担保しているため、ファイル `import` の「型不明の opaque」とは別概念である。

### 2.5 Async extern(v0.7 / M38)

`uses` に `Async` を含めれば、その `extern` 署名は非同期呼び出しとして扱われる。専用構文は無く、
§2.1 の `uses` 列にエフェクトとして書くだけ(M37 の `uses Async` と同じモデル)。
...
- 呼び出し元が `uses Async` を宣言していれば、生成 TS は自動で `await` を挿入する
  (`return await Net.fetch(url);`)。Kei ソースに `await` は登場しない(v0.7 / M37 の原則そのまま)。
- 呼び出し元が `uses Async` を宣言していなければ、他のエフェクトと同じく境界越しの `KEI-E3001` になる。
- **`extern query` に `Async` は付けられない。** query は同期・純粋観測子であることが定義そのものであり
  (§4.3)、`uses`(Async を含む)を付けると `KEI-E3005` で拒否される(query は `uses` を一切持てない
  という既存規則の帰結。Async だけの特例はない)。
