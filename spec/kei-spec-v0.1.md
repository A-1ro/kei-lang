...
1. ~~非同期の扱い~~ — **決着(v0.7 / M37・M38)**: `uses Async` エフェクトとして統合。`async function` + `Promise<T>` に写り、呼び出し側の `await` はコンパイラが emit 時に自動挿入する(§3.2 / §5参照)。M37 は関数コアのみを対象とし、`extern` async 署名と契約 async の e2e は M38 で扱う。
