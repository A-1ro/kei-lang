...
- **record 差分更新構文 spread(v0.5 / M32 / #97)**: `RecordName { ...expr, field: v }` の形で、既存の record 値の一部フィールドだけを変えた新しい値を構築できる。`...` による spread は 1 つのリテラルにつき最大1個、かつフィールドより前(先頭位置)にしか書けない(2個以上、または先頭以外への出現は構文エラー `KEI-E0101`)。spread を使えるのは **plain record リテラルのみ**。enum の named-field variant リテラル(`E.V { ... }`)では使えない(`KEI-E2004`。enum 値は TS 側で `{ kind, fields }` の 2 層構造に写るため object spread の意味論が成立せず、また型システムが variant を保持しないため variant 一致を静的に検証できない)。spread 式の型は、そのリテラルが構築しようとしている record 型と一致していなければならない(不一致は `KEI-E2001`)。spread 式の型が tagged 型の場合は基底 record をそのまま spread できず `KEI-E2005`(distinct tagged types do not mix と同じ tagged 混同エラー)になる(tagged の再構築規則に従い、tagged 値の生成は常に同名コンストラクタ呼び出し `Tag(value)` を経由する)。spread 式の型が解決できない場合(opaque import 由来の `Ty::Unknown` など)、spread の型照合とフィールド充足の検査はいずれも行われない(他の Unknown 経路と同じ gradual typing の一般規則に従う)。明示フィールドは spread 由来の値を上書きし、spread がある場合はフィールド網羅チェック(missing field)を行わない。契約式(`requires` / `ensures`)内でも spread を使用できる(spread 式自体も契約式の純粋性制約に従う)。TS 出力は object spread(`{ ...expr, field: value }`)に写す。
...
### 2.5 コンビネータ引数位置限定ラムダ(v0.4 / M25 / #59、v0.5 / M31 で読み取り専用キャプチャ許可)
...
- **構文**: `p => expr`(単項)/ `(a, b) => expr`(複数)。**body は単一式**。`{ ... }` ブロック禁止(M25 段階の射程)。**0 引数 `() => expr` は不許可**(`KEI-E0101`、F0)— コンビネータの arity が 0 を期待しないため、構文段階で弾く。

  補足(M31): 読み取り専用キャプチャの導入後も 0 引数は禁止のまま。0 引数ラムダは各要素に対する述語・射影として意味を持たず(コンビネータは常に per-element の値をラムダへ渡す)、キャプチャ変数のみを参照する定数的な式であってもコンビネータの用途に合わないため。
...
- **キャプチャ(v0.5 / M31): 読み取り専用キャプチャ可**。ラムダ body 内から参照できるのは
  **ラムダパラメータ + 囲みスコープの `let` 束縛 / 関数パラメータ(読み取りのみ)+ トップレベル関数 + import** —
  通常のレキシカルスコープと同じ innermost-first で解決する。Kei にはそもそも再代入が無いため、
  「読み取りのみ」は静的に健全(キャプチャ後に値が変わって観測結果がずれる余地が無い)。
  シャドーイング規則: ラムダパラメータが囲みスコープの変数と同名なら、内側(ラムダパラメータ)が勝つ
  (通常のスコープ規則どおり)。ネストしたラムダ(§ 末尾「ネスト可能」参照)からは、内側ラムダの body で
  外側ラムダのパラメータも読み取り参照できる(これも通常のレキシカルスコープに揃えるための意図的な挙動)。
  唯一の例外は **`result`**(ensures の特殊束縛): lambda 内からは引き続き見えない
  (F1 / 「`result` is not accessible from lambda bodies」、issue #113 で拡張を検討中)。
...
- **`old(...)` in lambda(v0.5 / M31 で規則を狭めた)**: `old` は関数入口で 1 回評価される(emit が `kei$old$N` に bind する)のに対し、lambda body は呼び出しごとに評価される — 時相が原則整合しないため、**引数がキャプチャ変数(ラムダパラメータではなく、囲み関数の `let` / パラメータ)だけを参照する式**(Field / Unary / Binary / リテラルとの組合せ可、`Call` 部分式なし)であれば、その値は関数実行中に変化しないため入口での 1 回評価がそのまま安全に成立する ── これは**許可**する(`ensures items.all(i => i.qty <= old(limit))` のように書ける)。一方、次のいずれかに該当する `old(arg)` は引き続き `KEI-E4002`(N3 / [0]、ensures モード限定)で禁止する:
  - `arg` がラムダパラメータ(ネスト時は外側ラムダのパラメータも含む)を参照する(例: `old(p.qty)` — per-element に評価される値を関数入口の 1 回評価に固定してしまう)
  - `arg` が `Call` 部分式を含む(例: `old(Database.maxLimit())` — 呼び出し結果を lambda 内で再評価すべきか静的に判断できない)
...
Async              // 非同期呼び出し。IO 傘下ではない独立ルート(v0.7 / M37)
```

- エフェクトは階層を持つ。`uses IO` は全IOの包括許可(雑だが合法)。細かいほど合意書として価値が高い。
- **例外: `Async` は `IO` の子ではない独立ルート。`uses IO` は `Async` を包含しない**(IO 宣言関数が黙って非同期になる互換性破壊を避けるため。v0.7 / M37)。非同期呼び出しを行うには明示的に `uses Async` の宣言が必要。
...
- **非同期の扱い(v0.7・M37/M38で導入)**: `uses Async` 関数は `async function` + `Promise<T>` に写る。呼び出し側は自動 await(呼び出し側の Kei コードに `await` 演算子は登場せず、コンパイラが emit 時に挿入する)。
...
1. ~~非同期の扱い~~ — **決着(v0.7 / M37・M38)**: `uses Async` エフェクトとして統合。`async function` + `Promise<T>` に写り、呼び出し側の `await` はコンパイラが emit 時に自動挿入する(§3.2 / §5参照)
