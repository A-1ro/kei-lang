...
>
> **v0.4 / M25(#59)で、案2 にコンビネータ引数位置限定の純粋ラムダ(`p => expr` / `(a, b) => expr`)を追加した。** 純粋限定・引き続き第一級関数値は禁止(`let f = (ラムダ)` は `KEI-E2001`)。**v0.5 / M31 で読み取り専用キャプチャを許可**(囲みスコープの `let` / 関数パラメータをラムダ body から参照可能。§5.1 の `any(e => e.field == outerVar)` の例を参照)。`examples/collections/inventory.kei` 現行版はラムダ表記を canonical な参照例として示している。本仕様の v0.1 §2.5 を併読のこと。
...
`xs.map(e => e.field).contains(value)` でスカラーフィールドを先に取り出してから
比較する方向へ誘導する(構造等価の言語サポートが入るまでの回避策も `==` と共通)。
`any(e => e.field == outerVar)` のように囲み関数の `let` / パラメータ(`outerVar`)を
ラムダ内で直接比較する形は、**v0.5 / M31 で読み取り専用キャプチャが許可された**ため
書ける(`outerVar` はラムダ body から読み取り参照できる。§2.5 参照)。`map` のラムダは
自パラメータ + キャプチャ変数を参照でき、`contains` の引数(`value`)はラムダの外側で
普通の式として評価される。
