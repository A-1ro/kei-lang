...
`any(e => e.field == outerVar)` のように外側の変数をラムダ内で直接比較する形は、
ラムダのキャプチャ禁止(v0.4 / M25、`KEI-E2001`)に抵触するため書けない。
`map` のラムダは自パラメータのみを参照するのでこの制約に触れず、`contains` の
引数(`value`)はラムダの外側で普通の式として評価されるため問題ない。
