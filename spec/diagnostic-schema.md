...
> spec/pact-spec-v0.1.md §6.2 の「案」を M0 で確定させたもの。
> **Diagnostic 型の唯一の定義元は `pact_check` クレート**(ARCHITECTURE.md 不変条件 1)。
...
形式: `PACT-E[カテゴリ1桁][連番3桁]`(例: `PACT-E2042`)

| カテゴリ | 範囲 | 担当領域 |
|---|---|---|
| 0 | PACT-E0xxx | 字句・構文(lexer / parser) |
| 1 | PACT-E1xxx | 名前解決(未定義参照・重複定義・import) |
| 2 | PACT-E2xxx | 型検査 |
| 3 | PACT-E3xxx | エフェクト検査(uses 節) |
| 4 | PACT-E4xxx | 契約検査(requires / ensures の純粋性等) |
| 5 | PACT-E5xxx | フォーマッタ・トランスパイラ等ツール側 |
...
- `pact check --json` は Diagnostic の配列(`Diagnostic[]`)を出力する。
