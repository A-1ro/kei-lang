...
- `docs/kei-roadmap-v0.10.md` — v0.10 のテーマ契約(純ロジックは全部 Kei で書ける)。v0.9.0 後の実運用統合で判明した String 表現力のギャップ(`length` の UTF-16 意味論 #159 / stdlib の薄さ #160)を埋める版。**v1.0(実 `wrangler deploy`)は後ろ倒しし、v0.10 を手前に挿入**(オーナー決定)。詳細 /goal は 🤝 合意後に追記予定。傘 issue #162。運用は上と同じ
