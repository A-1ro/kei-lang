...
- `docs/kei-roadmap-v0.5.md` — v0.5 の /goal 契約書集(M28〜M34: API ロジックを書ける表現力 + MCP 検証経路)と **v1.0 到達戦略**(v1.0 = Cloudflare Workers に Hono を使用した API としてデプロイできること。v0.6 npm import → v0.7 async → v0.8 HTTP/JSON 境界 → v0.9 Workers テンプレート)。運用は上と同じ
- `docs/kei-roadmap-v0.6.md` — v0.6 の /goal 契約書集(M35〜M36: extern package による npm bare specifier import。段階1は namespace import のみ、パッケージ解決は TS エコシステムの責務、**全完了**)。運用は上と同じ
- `docs/kei-roadmap-v0.7.md` — v0.7 の /goal 契約書集(M37〜M38: `uses Async` エフェクト + async 関数コア + 境界統合、**全完了**)と **設計原則**(Async は色ではなくエフェクト / await 演算子を持たない / Promise 型は Kei に露出させない / 契約式は同期・純粋のまま / sequential のみ、並列は v0.8+ 🤝)。実装エージェントは着手前に設計原則節を熟読必須。運用は上と同じ
- `docs/kei-roadmap-v0.8.md` — v0.8 の /goal 契約書集(M39: @kei/hono アダプタ + HTTP/JSON 境界 + e2e。v1.0 blocker 3/3。**言語機能変更ゼロの版** — Hono は named export で Kei ソースから直接 import 不要、アダプタ層 `@kei/hono` の TS 側で `import { Hono } from 'hono'` すれば足りる)と **設計原則**(言語機能追加は原則ゼロ / Hono Context を Kei に引き込まない / JSON は record 型ごとの専用 extern / Workers 特有型は Kei に露出させない)。実装エージェントは着手前に設計原則節を熟読必須。運用は上と同じ
- `docs/kei-roadmap-v0.9.md` — v0.9 の /goal 契約書集(M40〜M43: Workers テンプレート & デプロイ。**🤝 6 点合意済み(2026-07-13)** — M40 で file: 依存 × wrangler bundling + wrangler dev 実挙動の疎通を最優先で確認(NG 時のみ npm 公開前倒し / テンプレート内コピーを人間判断)/ M41 は issue #136 の pathParams 追加 + String stdlib 部分拡張(split・indexOf)+ TS closure DI + **契約違反は requires/ensures とも 500 に統一(M39 実装の 400 は修正対象)** / M42 は在庫 API 題材・wrangler.jsonc / M43 は vitest-pool-workers で CI 常設)と **設計原則**(Workers bindings/env は Kei に露出させない / デプロイ物は Kei=ロジック・TS=エントリポイント・wrangler=bundling の 3 層分担 / workerd 依存テストは cargo test を人質に取らない)。実装エージェントは着手前に設計原則節を熟読必須。運用は上と同じ
- `docs/kei-roadmap-v0.10.md` — v0.10 の /goal 契約書集(M44〜M47: 純ロジックは全部 Kei で書ける)。v0.9.0 後の実運用統合で判明した String 表現力のギャップ(`length` の UTF-16 意味論 #159 / stdlib の薄さ #160)を埋め、独立 I/O の並行 async(#161)を最小形で足す版。**v1.0(実 `wrangler deploy`)は後ろ倒しし、v0.10 を手前に挿入**(オーナー決定)。M44 code point 意味論 → M45 stdlib 段階2 + 正規表現の態度 → M46 純ロジック等価テスト実証(テーマ合否ゲート)→ M47 並行結合子。**🤝 5 点合意済み(2026-07-27)** — `length`(UTF-16)温存 + `codePointCount()` 追加(加法)/ stdlib は high tier + `contains`(pad・repeat は v1.x)/ 正規表現は言語に入れず定石例示 + extern 境界 / 並行は同種リスト結合子 `parallel`(emit は `Promise.all`)/ 順序は String 先行。傘 issue #162。運用は上と同じ
