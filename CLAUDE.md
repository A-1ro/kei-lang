...
- `docs/kei-roadmap-v0.9.md` — v0.9 の /goal 契約書集(M40〜M43: Workers テンプレート & デプロイ。**叩き台・🤝 未合意** — M40 で file: 依存 × wrangler bundling の疎通を最優先で確認し、NG なら npm 公開前倒し / テンプレート内コピーの分岐を人間判断。issue #136 のパスパラメータ抽出・依存注入経路と契約違反→HTTP ステータス写像・CI での wrangler の扱いはすべて 🤝 事前合意制)と **設計原則**(Workers bindings/env は Kei に露出させない / デプロイ物は Kei=ロジック・TS=エントリポイント・wrangler=bundling の 3 層分担 / wrangler 依存テストは cargo test を人質に取らない)。実装エージェントは着手前に設計原則節を熟読必須。運用は上と同じ
