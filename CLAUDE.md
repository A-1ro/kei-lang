...
- `docs/kei-roadmap-v0.7.md` — v0.7 の /goal 契約書集(M37〜M38: `uses Async` エフェクト + async 関数コア + 境界統合)と **設計原則**(Async は色ではなくエフェクト / await 演算子を持たない / Promise 型は Kei に露出させない / 契約式は同期・純粋のまま / sequential のみ、並列は v0.8+ 🤝)。実装エージェントは着手前に設計原則節を熟読必須。運用は上と同じ
