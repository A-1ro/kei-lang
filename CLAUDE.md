...
- `docs/kei-roadmap-v0.8.md` — v0.8 の /goal 契約書集(M39〜M40: extern package 段階2 default export + @kei/hono アダプタ + HTTP/JSON 境界。v1.0 blocker 3/3)と **設計原則**(言語機能追加は最小、複雑さはアダプタ層に閉じ込める / Hono Context を Kei に引き込まない / default のみ解禁、named import は先送り / Workers 特有型は Kei に露出させない)。実装エージェントは着手前に設計原則節を熟読必須。運用は上と同じ
