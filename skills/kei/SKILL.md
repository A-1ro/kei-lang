...
戻り値型を明示した `noHeaders()` でラップしておくのが定型:
...
(`tests/cli/projects/app/http_health.kei`):
...
(`tests/cli/projects/app/http_users.kei`)。`match` で `Option<String>` の `bodyText` と
`Option<UserRequest>` のパース結果を順にたどり、どちらかが `None` なら 400 系の応答を返す:
  ...
  uses Async
{
  return match req.bodyText {
    Some(text) => match parseApp.parseUserRequest(text) {
