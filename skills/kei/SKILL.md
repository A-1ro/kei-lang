...
- **文字列 stdlib 段階1(v0.5 / M30)**: `String + String -> String`(混在は `KEI-E2001`。tagged String 同士・tagged と素の String の連結も `+` では不可、`KEI-E2001`)。`s.length -> Int`(UTF-16 code unit 長)。`s.toInt() -> Option<Int>`(受理は `^-?[0-9]+$` かつ安全整数範囲、それ以外は `None`。注: `toInt()` を含む契約は `--generative` では bounded 評価器の制約により `[runtime]` のまま。issue #109)。`n.toString() -> String`。`s.split(delimiter) -> List<String>`・`s.indexOf(needle) -> Option<Int>`(v0.9 / M41)。すべて契約式内でも使える。
- **String は code point 単位まで保証する(v0.10 / M44)。grapheme と正規化は境界の外**(`spec/kei-spec-v0.10-strings.md` / 実例 `examples/strings/codepoints.kei`)。
  - **文字数は `s.codePointCount() -> Int` で数える**(`"a😀b".codePointCount() == 3`)。`s.length` は UTF-16 code unit 長で、絵文字(サロゲートペア)を 2 と数えるので「人間の感覚どおりの文字数」には `codePointCount()` を使う。`codePointCount()` は純粋で契約式内でも使え、`--generative` の bounded 評価器も評価できる(`length` との差はサロゲート境界値で PBT に現れる)。
  - **「1 文字ずつ」処理は `s.split("")`(空デリミタ = code point 単位)+ `List` 畳み込み**で書く(新しい for 構文は無い)。`"a😀b".split("") == ["a", "😀", "b"]`(サロゲートを割らない)。emit は runtime helper 経由で `Array.from(s)` と等価。
...
- **正規表現は言語に入れない(v0.10 / M45)。** slug 生成・タグ正規化などは「小文字化 → `split("")` で 1 code point ずつ → 許可集合で畳み込み(除去 / 置換 / 判定)」の定石で書く(`s.startsWith` / `endsWith` / `contains` / `substring` / `trim` と組み合わせる)。どうしても正規表現が要るパターンは extern で TS 側 `RegExp` に出す(`spec/kei-spec-v0.10-strings.md` §5 に定石と境界)。
