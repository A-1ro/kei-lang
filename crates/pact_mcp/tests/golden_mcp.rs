...
//! `{name}.response.json`(期待レスポンス)のペア。[`pact_mcp::Server::handle`]
//! にリクエストを渡し、返ったレスポンス Value を期待 JSON と構造比較する
//! (プロセス起動なしでリクエスト→レスポンスを検証)。
//!
//! 期待ファイルの再生成: `UPDATE_GOLDEN=1 cargo test -p pact_mcp --test golden_mcp`
...
fn tools_list_exposes_four_tools() {
    ...
    assert_eq!(
        names,
        vec!["pact_spec", "pact_check", "pact_fmt", "pact_examples"],
