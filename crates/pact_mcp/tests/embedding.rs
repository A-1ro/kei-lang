...
//! 変わり、`pact_spec` / `pact_examples` の応答も必ず変わる。`include_str!` は
//! 対象ファイルを rustc の依存に登録するので、`cargo test` は spec 編集後に
//! 必ず pact_mcp を再コンパイルしてからこのテストを走らせる。
//!
...
fn embedded_examples_match_disk() {
    let disk = read_tree(&workspace_root().join("examples"), "pact");
...
#[test]
fn pact_spec_serves_current_error_doc() {
    let server = Server::new();
    let disk = fs::read_to_string(workspace_root().join("spec/errors/PACT-E3001.md"))
        .expect("error doc exists");

    let request = json!({
        "jsonrpc": "2.0", "id": 1, "method": "tools/call",
        "params": { "name": "pact_spec", "arguments": { "topic": "PACT-E3001" } }
    });
    let response = server.handle(&request).expect("pact_spec responds");
    let text = served_text(&response);

    assert_eq!(
        text, disk,
        "pact_spec must serve the on-disk error doc verbatim"
    );
...
#[test]
fn pact_examples_serves_current_example_body() {
    let server = Server::new();
    let disk = fs::read_to_string(workspace_root().join("examples/contracts/withdraw.pact"))
        .expect("example exists");

    let request = json!({
        "jsonrpc": "2.0", "id": 1, "method": "tools/call",
        "params": { "name": "pact_examples", "arguments": { "query": "withdraw" } }
    });
    let response = server.handle(&request).expect("pact_examples responds");
    let text = served_text(&response);

    assert!(
        text.contains(disk.trim_end()),
        "pact_examples must serve the on-disk example body verbatim"
    );
}
