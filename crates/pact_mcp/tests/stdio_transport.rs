//! stdio トランスポートの End-to-End 検証。実バイナリ `pact-mcp` を起動し、
...
fn server_starts_and_answers_over_stdio() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_pact-mcp"))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn pact-mcp");
    ...
    let requests = concat!(
        ...
        "\n",
        r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"pact_fmt","arguments":{"source":"module demo\n\nfunc double(x: Int) -> Int {\n  return x + x\n}\n"}}}"#,
        "\n",
    );
    child
        .stdin
    ...
    assert_eq!(init["id"], 1);
    assert_eq!(init["result"]["serverInfo"]["name"], "pact-mcp");
