...
//!
//! 言語処理は pact_check / pact_fmt / pact_syntax に委譲し、ここでは
//! 埋め込み spec/examples の取り回しと検査結果の整形だけを行う
//! (ARCHITECTURE.md: pact_mcp は言語処理ロジックを持たない)。

use crate::embedded;

/// インライン source に与える合成ファイル名。Diagnostic の span.file に入る。
pub const SYNTHETIC_FILE: &str = "source.pact";

const MAIN_SPEC: &str = "pact-spec-v0.1.md";
...
/// - `PACT-Exxxx` … `spec/errors/{code}.md`
...
fn spec_index() -> String {
    let mut s = String::from("# Pact spec index\n\n");
    s.push_str(
        "`pact_spec` の `topic` にセクション番号・見出しキーワード・エラーコードを渡すと\n\
         該当箇所を返す。topic を空にするとこの索引を返す。\n\n",
    );
    s.push_str("## セクション (pact-spec-v0.1.md)\n\n");
    if let Some(main) = embedded::spec_file(MAIN_SPEC) {
...
pub fn run_check(source: &str) -> ToolOutcome {
    ...
    let mut diags = pact_check::syntax_diagnostics(SYNTHETIC_FILE, &parsed.errors);
    // 構文エラーがあるときは壊れた AST に意味検査をかけない(golden_check と同方針)。
    if parsed.errors.is_empty() {
        diags.extend(pact_check::check_module(SYNTHETIC_FILE, &parsed.module));
    }
...
/// (壊れた入力を「それらしく」書き換えない — pact_fmt の方針)。
pub fn run_fmt(source: &str) -> ToolOutcome {
    match pact_fmt::format_source(source) {
        Ok(formatted) => ToolOutcome::ok(formatted),
        Err(errors) => {
            let diags = pact_check::syntax_diagnostics(SYNTHETIC_FILE, &errors);
            ToolOutcome::err(diagnostics_json(&diags))
        }
    }
}

fn diagnostics_json(diags: &[pact_check::Diagnostic]) -> String {
...
fn examples_index(files: &[(&str, &str)]) -> String {
    let mut s = String::from("# Pact examples\n\n");
    s.push_str(
        "`pact_examples` の `query` にキーワードを渡すと該当する例を返す\n\
