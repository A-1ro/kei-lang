...
/// サーバー名(serverInfo)。
pub const SERVER_NAME: &str = "pact-mcp";
/// サーバーバージョン(Cargo パッケージ版数)。
pub const SERVER_VERSION: &str = env!("CARGO_PKG_VERSION");

/// Pact MCP サーバー。状態は埋め込み静的データのみで、インスタンスは空。
...
fn tools_call(id: Option<Value>, params: &Value) -> Value {
    ...
    let outcome = match name {
        "pact_spec" => tools::run_spec(str_arg("topic").unwrap_or("")),
        "pact_check" => match str_arg("source") {
            Some(src) => tools::run_check(src),
            None => tools::missing_arg("source"),
        },
        "pact_fmt" => match str_arg("source") {
            Some(src) => tools::run_fmt(src),
            None => tools::missing_arg("source"),
        },
        "pact_examples" => tools::run_examples(str_arg("query").unwrap_or("")),
...
fn tools_list_result() -> Value {
    json!({
        "tools": [
            {
                "name": "pact_spec",
                "description": "Look up the Pact language spec. Pass `topic` as a section number (e.g. \"3\"), a heading keyword, or an error code (e.g. \"PACT-E3001\"); omit it for the index.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "topic": {
            ...
            {
                "name": "pact_check",
                "description": "Statically check Pact source (syntax + types + effects + contracts). Returns a Diagnostic[] JSON array, each with span, code, and at least one fix candidate.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "source": { "type": "string", "description": "Pact source text to check." }
                    },
                    "required": ["source"],
                    "additionalProperties": false
                }
            ...
            {
                "name": "pact_fmt",
                "description": "Format Pact source into canonical form. On a syntax error it does not reformat and returns the Diagnostic[] instead (isError).",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "source": { "type": "string", "description": "Pact source text to format." }
                    },
                    "required": ["source"],
                    "additionalProperties": false
                }
            ...
            {
                "name": "pact_examples",
                "description": "Search Pact example snippets by keyword (matches path and body). Omit `query` to list all examples.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
