...
(no actionable patterns — 今回の hook 発火は `tool_input.command` に `gh pr checks 132 --watch --interval 30 ... && gh pr merge 132 --squash --delete-branch --admin ...` と真正な `gh pr merge <N>` を含む複合コマンドだったため、初めて意図通りの契機で起動した。しかし `gh api repos/:owner/:repo/pulls/132/comments`(inline review)/ `issues/132/comments`(general discussion)/ `gh pr view 132 --json reviews,reviewDecision` の 3 経路すべてが空配列と `{"reviewDecision":"","reviews":[]}` を返し、レビュー由来の教訓化材料はゼロ。M39 の @kei/hono アダプタ + HTTP/JSON 境界 e2e という「言語機能変更ゼロ」の PR タイトル通り、admin merge (`--admin`) で CI の `test` job が `fail` 状態(https://github.com/A-1ro/kei-lang/actions/runs/29094848879/job/86368779648 で 36s で失敗)のままバイパスされている点だけが観察事実だが、これは reviewer 由来の指摘ではなく owner 判断のフローなので、教訓化スコープ外。)

メタ観察(:436 以降通算 15 回目 の同種メタ観察 — レビュー由来ではないため正式教訓化はしない):
- **今回は改修候補 (a) の `Bash(gh pr merge*)` matcher 限定が働いていたと仮定した場合でも、この複合コマンド `gh pr checks ... && gh pr merge ...` はマッチして子セッションを起動する**という点で、(a) 単独では誤発火除去と正発火保持の両立が可能だと再確認できた。逆に言えば、改修候補 (a) を入れるだけで、:517 以前に頻発していた「Kei 言語の挙動確認 Bash」「`git log` 系」「`cargo test` 系」による誤発火をほぼゼロにできる見込みで、(b) の prompt 冒頭 early-exit は保険として残せば充分。7 回連続の予告となっている改修候補は、次に `.claude/settings.json` を触る Opus セッションで **その場で片付ける**べきで、これ以上の「予告のみ」エントリを積むのは無駄。
- **`--admin` merge で CI 失敗をバイパスして merged される PR が今回発生した**ことは、post-merge lessons 側の教訓ではなく release-bump / M39 側の handoff-candidates.md に落とすべき運用観察。test job の失敗内容が M39 の HTTP/JSON e2e 変更に起因するリグレッションか、既存の flaky test か、または merge タイミングでの環境要因かは、この hook からは判定不能(hook 子セッションのスコープは PR #132 レビュー内容の抽出のみ)。次に main を触る Opus は `gh run view 29094848879 --log-failed` で内容を確認して、リグレッションなら hotfix PR、flaky なら quarantine、環境要因ならスルーの判断を下す必要がある。

## PR #132: feat: M39 @kei/hono アダプタ + HTTP/JSON 境界 e2e(言語機能変更ゼロ) — 2026-07-10 merged (hook re-run: CI 失敗テストのローカル再現 `cargo test` による誤発火)

(no actionable patterns — 今回の hook 発火は `tool_input.command` が `rm -rf tests/cli/packages/parse-app/node_modules ... 2>/dev/null; cargo test --package kei_cli --test cli app_greeter_hello_build_and_test kei_test_builds_then_runs_contracts --quiet 2>&1 | tail -8; echo TEST=$?` という **`gh pr merge` を全く含まない、admin merge 後の CI fail 再現用ローカル `cargo test`** だった。:534 で予告した「次に main を触る Opus は `gh run view 29094848879 --log-failed` で内容を確認」の派生作業として cargo test を直接叩いた結果、副産物として PostToolUse hook が誤発火した形。直近マージ PR は依然 #132 のまま、`gh api pulls/132/comments` / `issues/132/comments` / `gh pr view 132 --json reviews,reviewDecision` を再確認しても :530 で記録済みの「3 経路すべて空配列」から増分ゼロ。教訓化する材料なし。tool_output も `error: unexpected argument 'kei_test_builds_then_runs_contracts' found` で失敗しているが、これは cargo test の CLI 引数(テスト名を 1 個ずつ渡すか `--` 区切りが要る)の理解ミスであり、レビュー由来の教訓ではない。)

メタ観察(:436 以降通算 16 回目 の同種メタ観察 — レビュー由来ではないため正式教訓化はしない):
- **改修候補 (a)+(b) の 8 回連続予告**。今回のように `cargo test` のようなごく一般的なコマンドで PostToolUse hook が発火してしまうと、(1) 子セッション起動コスト、(2) `gh api pulls/*/comments` × 3 の余計な GitHub API 消費、(3) この `(no actionable patterns)` エントリで lessons-from-reviews.md が肥大化して SessionStart hook が Opus に流す context を圧迫、という 3 重の無駄が発生する。特に (3) は Opus セッション全体のトークン予算に効くので、`.claude/settings.json` の PostToolUse matcher を `Bash(gh pr merge*)` に限定する改修は **次に settings.json を触る Opus セッションで最優先で片付ける**。予告 8 回目でも実装されていないのは、これまでの Opus セッションが :525 / :526 / :533 のメタ観察を読まずに main タスクに集中しているためと推測される。SessionStart hook が流す「直近 N 件」の N を大きめに取るか、あるいは `docs/dev-notes/handoff-candidates.md` 側に「settings.json 改修 TODO」として別途登録するのが確実(この hook 子セッションからは他ファイル改変禁止なので実行できない)。
- **cargo test の引数エラー `error: unexpected argument 'kei_test_builds_then_runs_contracts' found`** は、`cargo test --test cli` が受け取れるフィルタ引数は 1 個までで、2 個目以降は `--` 区切りが必須という cargo の CLI 仕様。次に CI fail を再現する Opus は `cargo test --package kei_cli --test cli app_greeter_hello_build_and_test` と `cargo test --package kei_cli --test cli kei_test_builds_then_runs_contracts` を **2 回に分ける** か、`cargo test --package kei_cli --test cli -- app_greeter_hello_build_and_test kei_test_builds_then_runs_contracts` と **`--` 以降にまとめる** べき(ただし後者は「両方にマッチする substring」ではなく OR フィルタとして動くかは要検証)。これも post-merge lessons のスコープ外(reviewer からの指摘ではない Kei/cargo 運用知識)なので正式教訓化はしないが、`docs/dev-notes/` 配下のどこかに落としておくと後続 Opus のトリップアップが減る。

## PR #132: feat: M39 @kei/hono アダプタ + HTTP/JSON 境界 e2e(言語機能変更ゼロ) — 2026-07-10 merged (hook re-run 2: `cargo test` 2 回連続実行による誤発火 / CI fail テストがローカルで両方 pass)

(no actionable patterns — 今回の hook 発火は `tool_input.command` が `cargo test --package kei_cli --test cli app_greeter_hello_build_and_test --quiet 2>&1 | tail -5; echo T1=$?; cargo test --package kei_cli --test cli kei_test_builds_then_runs_contracts --quiet 2>&1 | tail -5; echo T2=$?` という、:542 のメタ観察で「2 回に分ける」と処方した通りの **2 回連続 `cargo test`**。これも `gh pr merge` を全く含まないので post-merge lessons スコープ外だが、直近マージ PR は依然 #132 のまま、`gh api pulls/132/comments`(0 件)/ `issues/132/comments`(0 件)/ `gh pr view 132 --json reviews,reviewDecision`(`{"reviewDecision":"","reviews":[]}`)を再々確認しても増分ゼロ。教訓化材料なし。)

メタ観察(:436 以降通算 17 回目 の同種メタ観察 — レビュー由来ではないため正式教訓化はしない):
- **改修候補 (a)+(b) の 9 回連続予告**。前回 :541 で「次に settings.json を触る Opus セッションで最優先で片付ける」と書いたが、その直後の hook 再発火(=同じ Opus セッションが CI 失敗テストの詳細調査を継続中)でまた `(no actionable patterns)` エントリが積まれた。この構造は **hook 発火 → 子セッションが lessons-from-reviews.md に追記 → 追記した本人(親 Opus)は追記結果を見ずに次のコマンドを叩く → また hook 発火** という無限ループ的性質を持つため、Opus 側で気付く契機がそもそも存在しない。改修候補 (a)(`.claude/settings.json` の PostToolUse matcher を `Bash(gh pr merge*)` に限定)は今後の Opus セッションが settings.json を触ったタイミングで **必ず併せて実装**するのが唯一の脱出路。この教訓化フォーマットに `handoff-candidates.md` に自動追記する仕組みは無いので、次に main タスク以外の余裕がある Opus が明示的に転記するしかない。
- **CI で fail していた `app_greeter_hello_build_and_test` と `kei_test_builds_then_runs_contracts` は今回ローカルで両方 pass**(T1=0, T2=0, それぞれ 2.78s と 1.65s で 1 passed / 0 failed)。ローカル pass × CI fail の乖離は、(1) CI 側の環境差(Node バージョン、npm レジストリキャッシュ、`node_modules` の残存状態)、(2) flaky test、(3) main への他 PR の merge タイミング差 のいずれか。次に CI fail を追う Opus は `gh run view 29094848879 --log-failed` の実出力と今回のローカル run の差分を突き合わせるべきで、特に `parse-app` の `node_modules` 事前クリーンアップ(:538 の `rm -rf tests/cli/packages/parse-app/node_modules`)が結果を変える点は要注目。ただしこれも post-merge lessons のスコープ外なので、正式教訓化ではなく handoff-candidates.md 側に運用ノートとして残すのが正解。
