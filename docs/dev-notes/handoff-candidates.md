...
## PR #132: feat: M39 @kei/hono アダプタ + HTTP/JSON 境界 e2e(言語機能変更ゼロ)— 2026-07-10 merged

### Candidate: HTTP アダプタは「境界でだけ変換する」— Context を Kei に渡さない
**Why this matters for HANDOFF.md**: 将来「なぜ @kei/hono は Hono の Context をそのまま Kei ハンドラに渡さないのか」を必ず問われる。答えは Kei 側の設計上の 2 制約(エフェクト封じ込め・record は値型)から演繹的に導かれるので、忘れると「便利だから Context を record に足そう」という誤った提案に押し切られる。
**Draft entry**(lift verbatim if approved):
> `@kei/hono`(`tests/cli/packages/kei-hono/src/index.ts`)は Hono の `Context` を Kei ハンドラに直接渡さず、境界で平坦な `HttpRequest` record に組み立て直し、戻り値 `HttpResponse` から標準 `Response` を再構築する。理由は 2 つ: (1) `Context` は `env` / `executionCtx` / `var` など Cloudflare Workers 実行時の広い面を持ち、そのまま渡すと Kei のエフェクトシステムが把握できない副作用の抜け道になる、(2) Kei の record は構造的な値型で、メソッド持ちオブジェクトの `Context` を表現するには不向き。この 2 点は仕様なので、将来「Context をそのまま record に足す」提案が来ても却下する。Kei ハンドラ本体は Hono にも `fetch` にも依存しない純粋な `HttpRequest -> HttpResponse` record 変換関数のまま書ける、というのが M39 の設計目標。

### Candidate: `fromContext` は `bodyText` を常に `Some(bodyText)` で返す(空文字列を潰さない)
**Why this matters for HANDOFF.md**: 「body なしなら `None` にしよう」と直感で最適化すると、GET と POST 空 body の意味が Kei 側で区別できなくなる境界層バグを再導入する。M39 レビューで一度この情報損失(finding [2])を封鎖したので、履歴に残しておかないと同じ提案が繰り返される。
**Draft entry**(lift verbatim if approved):
> `@kei/hono` の `fromContext(c)` は `c.req.text()` の結果を常に `Some(bodyText)` に包んで返す。GET のように body が無いリクエストは `Content-Length: 0` として `Some("")` になり、`None` にはならない。理由: 「body 無し」と「空文字列」を境界層で畳み込むと情報が失われ、Kei ハンドラ側で「本当に body が無かったのか、空文字列だったのか」の区別ができなくなる。空文字列を弾く判定は呼び出し側の Kei ハンドラで行う(SKILL.md §9 の `parseNonEmptyBody` パターン)。M39 実装初版は「body 無しは `None`」に潰していたが、レビュー finding [2] で「境界層で情報を捨てない」原則に沿って修正済み。

### Candidate: `mount()` は `Awaitable<HttpResponse>` を受ける — 同期ハンドラに `uses Async` を強要しない
**Why this matters for HANDOFF.md**: Kei のエフェクトシステム(`uses Async`)は「本当に非同期呼び出しを含むか」を型で表現する意味論を持つ。境界 API の型を `Promise<HttpResponse>` に固めるとその意味が鈍り、実際は同期のハンドラにまで `uses Async` が汚染する。M39 レビュー finding [1] で一度この設計判断を下したので、後で「型が緩いのが気持ち悪い」と `Promise<...>` に締めようとする提案を防ぐために残す。
**Draft entry**(lift verbatim if approved):
> `mount(app, method, path, handler)` の `handler` 型は `(req: HttpRequest) => Awaitable<HttpResponse>`(= 同期値 `HttpResponse` または `Promise<HttpResponse>`)にしてある。理由: Kei 側で `uses Async` を宣言していないハンドラは同期関数として TS に生成されるため、`mount` の型を `Promise<HttpResponse>` に固定すると、実際は非同期呼び出しを含まないハンドラにまで `uses Async` を強要してしまい、「本当に非同期か」を表す `uses Async` の意味が薄れる。実装は `await handler(req)` のままなので、同期値・`Promise` のどちらが返っても正しく動く。M39 実装初版は `Promise<HttpResponse>` に固めていたが、レビュー finding [1] で緩和済み。将来「型が緩いから締めよう」という提案が来ても、この意味論的な理由を優先する。

### Candidate: 契約違反(`KeiContractViolation`)は `mount()` で捕捉せず `app.onError` で中央処理する
**Why this matters for HANDOFF.md**: 「業務ロジックの 400」と「契約違反の 400」を同じ経路に流したくなるが、経路も body の形も違う。`mount()` の中で `try/catch` で 400 に潰すと、契約違反の可観測性(どの `requires`/`ensures` が違反したか)を失う。設計の分岐点を明示的に残しておかないと後で必ず「まとめて 400 にしよう」と潰される。
**Draft entry**(lift verbatim if approved):
> `@kei/hono` の `mount()` は Kei ハンドラが投げる `KeiContractViolation` を **捕捉せずそのまま投げる**。捕捉と 400 マッピングは呼び出し側が Hono の `app.onError` に集約する設計(SKILL.md §9 参照)。JSON parse 失敗による 400 と契約違反による 400 は別経路: 前者は Kei の `match Option<T>` が業務ロジックとして `badRequestResponse()` を直接返す(例外は発生しない)、後者は `requires` 違反で `KeiContractViolation` が例外として飛び、`onError` が 400 + `{ clause, condition }` に写す。どちらも HTTP 応答は 400 だが、経路も body の形も異なるので握り潰さずに書き分ける。`mount()` の中で `try/catch` すると契約違反の情報(どの節が違反したか)が失われるため禁止。

### Candidate: 汎用アダプタにアプリ固有関数を混ぜない — `parseUserRequest` はアプリローカル file: パッケージに置く
**Why this matters for HANDOFF.md**: Kei にジェネリック関数が無いので「record 型ごとに専用の `parseXxxRequest` extern」が必要になる。この関数群をどこに置くかで「@kei/hono に押し込む」「アプリ側に持たせる」の 2 案があり、M39 で明示的に選択肢 A(アプリローカル file: パッケージ)を採った理由を残しておかないと、将来「@kei/hono に `parseUserRequest` を足す方が便利」という提案が繰り返される。
**Draft entry**(lift verbatim if approved):
> Kei にジェネリック関数は無いため `Option<T>` を返す JSON パース関数は record 型ごとに専用 `extern` を書く必要がある。役割分担: `@kei/hono` が **汎用の** `parseAs<T>(text, shapeCheck) -> Option<T>`(TS 側 generic ヘルパー)だけを提供し、record 固有の shape check(例: `UserRequest.name` が string か)は **アプリローカルな file: パッケージ**が薄い wrapper として持つ(M39 では `tests/cli/packages/parse-app/`、依存チェーン `app → parse-app → @kei/hono → @kei/runtime`)。汎用アダプタ `@kei/hono` にアプリ固有 record 名を持ち込まないことで、他アプリが同じアダプタを使い回せる。M39 実装時に「@kei/hono の module 拡張(選択肢 B)」も検討したが、file: 依存ミニパッケージを `tests/cli/packages/` に集約する既存規約(ARCHITECTURE.md)と、汎用/固有の分離を優先して選択肢 A を採用した。

### Candidate: record field の `Map<K,V>` 初期化には `noHeaders()` 定型を使う(issue #133 回避策)
**Why this matters for HANDOFF.md**: Kei の型推論に record field 位置での期待型伝播漏れ(issue #133、KEI-E2012)があり、`HttpResponse { ..., headers: Map.empty(), ... }` と直接書くと型注釈要求エラーになる。回避策の定型を SKILL.md §9 に埋めたので、v1.x で type inference が直るまでは全 HTTP ハンドラで同じ形を強制する必要がある。忘れると「なぜ全ての HTTP fixture に `noHeaders()` があるのか」の答えが失われる。
**Draft entry**(lift verbatim if approved):
> `Map<String, String>` 型のフィールドを record リテラルの中で直接 `Map.empty()` にすると `KEI-E2012`(`'Map.empty()' requires a type annotation to determine its key/value types`)が発火する。これは record field 値への期待型伝播漏れで、kei_check 側の既知ギャップ(issue #133)。**回避策の定型**: 戻り値型を明示した `func noHeaders() -> Map<String, String> { return Map.empty() }` を各 HTTP fixture module に置き、record リテラルでは `HttpResponse { ..., headers: noHeaders(), ... }` と関数呼び出しで書く。v1.x で record field 位置の型伝播が直り次第、この定型は削除して直接 `Map.empty()` を書けるようにする想定。M39 の全 HTTP fixture(`http_health.kei` / `http_users.kei` / `http_model.kei`)がこの形を採っている理由はこの型推論ギャップの回避なので、v1.x で issue #133 を閉じる際に fixture も一括で書き直す。

### Candidate: 共有 npm パッケージの並行 install 競合は per-package Mutex(`npm_dist_lock`)で防ぐ
**Why this matters for HANDOFF.md**: `cargo test` は既定でスレッド並列実行するため、複数の `#[test]` が同じ npm パッケージ(`runtime` や `kei-hono`)を並行 install/build しようとして race する。`app_project_lock` は「同じプロジェクトディレクトリを触るテスト」の直列化しかカバーしないので、共有パッケージ側にも別レイヤの直列化が必要という設計判断が M36→M39 で段階的に発生した。忘れると「なぜテストが flaky に落ちるのか」の再デバッグが再燃する。
**Draft entry**(lift verbatim if approved):
> `crates/kei_cli/tests/cli.rs` の `ensure_npm_dist(root, pkg_rel, marker_rel)` は `pkg_rel` 単位の `Mutex`(`npm_dist_lock`、レジストリは `OnceLock<Mutex<HashMap<String, &'static Mutex<()>>>>`)で自身の呼び出しを直列化する。理由: `cargo test` はスレッド並列実行するため、複数の `#[test]` が共有 npm パッケージ(`runtime` / `tests/cli/packages/kei-hono` など)を並行 install / build しようとすると `npm install` が競合してテストが flaky になる。`app_project_lock()` は「`tests/cli/projects/app` 自身の dist 削除・npm install」だけを保護しており、共有パッケージ側の直列化は担当しない(`app` を触る 2 個の #[test] が同じ共有パッケージも触ると、`app_project_lock` を握った側だけが安全になる非対称になっていた)。M39 レビュー finding [3][4] で `ensure_runtime_built` / `ensure_kei_hono_built` を共通の `ensure_npm_dist` に統合し、per-package Mutex を汎用ヘルパー自身に埋めることで、呼び出し側が個別にロックを持たなくても race しない構造に変えた。新しい共有 npm パッケージを追加するときは `setup_npm_project` に `ensure_npm_dist(&root, "path/to/pkg", "path/to/pkg/dist/index.js")` を 1 行足すだけで OK(呼び出し側で Mutex を握らないこと)。

### Candidate: `build_golden_tree` の `wrote N module(s)` は module 数の source-of-truth — 更新は golden 再生成で
**Why this matters for HANDOFF.md**: `wrote 5 module(s)` → `wrote 8 module(s)` のような数値変更は「module 数を数え直す」という手動ステップに見えるが、実は fixture の `.kei` ファイル追加/削除に一意に対応する。この関係を忘れて「なぜここに 8 という数字があるか」が謎になると、golden 再生成手順の理解が壊れる。ARCHITECTURE.md 不変条件 3(golden 変更は人間レビュー必須)と紐付けて残す価値がある。
**Draft entry**(lift verbatim if approved):
> `crates/kei_cli/tests/cli.rs::build_golden_tree` は `run.stderr.contains("wrote N module(s)")` で `kei build` が **`tests/cli/projects/app/` 配下の `.kei` module を全部ビルドしたか** を検証する。この `N` は fixture の `.kei` ファイル数と 1:1 対応する(M36: 5 module → M39: 8 module = 既存 5 + `http_health.kei` + `http_users.kei` + `http_model.kei`)。fixture に `.kei` を追加/削除したときの更新手順: (1) `UPDATE_GOLDEN=1 cargo test -p kei_cli --test cli` で golden を再生成、(2) `wrote N` の `N` を新しい数に更新、(3) 既存の `expected/*.ts` / `*.ts.map` がバイト単位で不変であることを `cmp` で確認(ARCHITECTURE.md 不変条件 3: golden の変更は人間レビュー必須)、(4) 新規 `.kei` に対応する `expected/*.ts` / `*.ts.map` のみが追加されていることを確認。この数値だけを手で書き換えると「build されなかった module がある」ケースを検知できなくなるので、必ず golden 再生成とセットで更新すること。

## PR #132 (PostToolUse re-fire — CI 失敗ローカル再現 `cargo test` trigger): feat: M39 @kei/hono アダプタ + HTTP/JSON 境界 e2e(言語機能変更ゼロ)— 2026-07-10 merged

> **Note**: 本セッションの hook 発火トリガは `tool_input.command` が `rm -rf ... node_modules ...; cargo test --package kei_cli --test cli app_greeter_hello_build_and_test kei_test_builds_then_runs_contracts` の Bash 呼び出し(CI で落ちたテスト2本をローカル再現・修正確認)で、`gh pr merge` ではない。post-merge-handoff agent はプロンプト仕様に従い「`gh pr merge <N>` 未検出 → 最新 merged PR にフォールバック」で PR #132 を対象に走査したが、同 PR の設計判断候補は M39 マージ直後の元発火(上のセクション)で 8 件抽出済み(境界変換 / Some(bodyText) / Awaitable / onError / アプリローカル file: パッケージ / noHeaders / npm_dist_lock / wrote N)。**本発火では新規 candidate なし**(重複を避けるため追記スキップ)。
>
> 副次的観察(HANDOFF.md 候補ではなく hook 運用側の note): `cargo test --package kei_cli --test cli` に **複数の testname 位置引数** を渡すと `error: unexpected argument 'kei_test_builds_then_runs_contracts' found` が出る(cargo は `--test <name>` のあとの位置引数を最初のひとつしかフィルタ名として受け付けない)。ローカルで 2 本まとめて回したいときは `cargo test --package kei_cli --test cli 'app_greeter_hello_build_and_test|kei_test_builds_then_runs_contracts'`(1つの位置引数として regex 相当のフィルタを渡す)か、`--test cli -- name1 name2` のように `--` 以降に並べる形にする。今回のトリガコマンドは前者を守っておらず、`TEST=0` にはなったものの実際には `app_greeter_hello_build_and_test` の1本しか回っていない(cargo は不正引数で早期 exit する場合と、フィルタとして解釈できない位置引数でテスト0件マッチとして exit 0 する場合が混在し、`error:` を stdout に流しつつ `$?=0` になる挙動)。PR 目的の CI 再現としては片手落ちなので、`cargo test --package kei_cli --test cli` を無フィルタで回すか、上の regex 相当フィルタに書き直すのがトリガコマンドの正しい形。

(no design-decision candidates for this PR — this section documents the spurious PostToolUse re-fire only)

## PR #132 (PostToolUse re-fire #2 — CI 失敗 2 テストローカル確認 `cargo test` trigger): feat: M39 @kei/hono アダプタ + HTTP/JSON 境界 e2e(言語機能変更ゼロ)— 2026-07-10 merged

> **Note**: 本セッションの hook 発火トリガは `tool_input.command` が
> `cargo test --package kei_cli --test cli app_greeter_hello_build_and_test --quiet 2>&1 | tail -5; echo T1=$?; cargo test --package kei_cli --test cli kei_test_builds_then_runs_contracts --quiet 2>&1 | tail -5; echo T2=$?`
> の Bash 呼び出し(CI で落ちていた 2 テストをローカルで再度確認 — 両方 `T1=0` / `T2=0` で緑)であり、`gh pr merge` ではない。post-merge-handoff agent はプロンプト仕様に従い「`gh pr merge <N>` 未検出 → 最新 merged PR にフォールバック」で PR #132 を対象に走査したが、同 PR の設計判断候補は M39 マージ直後の元発火で 8 件抽出済み(境界変換 / Some(bodyText) / Awaitable / onError / アプリローカル file: パッケージ / noHeaders / npm_dist_lock / wrote N)、かつ 1 回目の re-fire でも重複回避のため追記スキップ済み。**本発火(re-fire #2)も新規 candidate なし**。
>
> 副次的観察(HANDOFF.md 候補ではなく hook 運用側の note): 今回のトリガコマンドは前回の re-fire で指摘した「複数 testname を1つの `cargo test` に渡すと片方しか回らない」問題を回避するため、**テストごとに `cargo test --package kei_cli --test cli <name>` を分けて 2 回起動 → それぞれ `T1=$?` / `T2=$?` で終了コードを別々に確認する形**になっていた(結果はどちらも `test result: ok. 1 passed; 0 failed`)。cargo の位置引数フィルタ制約に対する運用側の正しい回避パターンなので、CLAUDE.md か dev-notes 側に手順として残すと将来 CI 失敗ローカル再現時に迷わない(ただし HANDOFF.md 候補としては「Kei プロジェクトの why」ではなく「cargo test の使い方」なので、こちらではなく `docs/dev-notes/lessons-from-reviews.md` か CLAUDE.md「テスト実行のコツ」節が置き場所として妥当)。

(no design-decision candidates for this PR — this section documents the second spurious PostToolUse re-fire only)
