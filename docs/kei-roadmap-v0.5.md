...
- spread 式の型は構築先の型(plain record ならそのレコード型、enum の named-field variant ならその enum 型)と完全一致していること(不一致は `KEI-E2001`)。存在しないフィールドの上書きは引き続き `KEI-E2002`。spread がある場合は missing-field 診断を出さない。全フィールドを明示上書きした場合の warning は出さない(正規形は kei_fmt が決める)。
